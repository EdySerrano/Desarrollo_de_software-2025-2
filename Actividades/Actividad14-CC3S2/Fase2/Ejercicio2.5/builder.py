...
class InfrastructureBuilder:
    ...
    def build_group(self, name: str, size: int) -> "InfrastructureBuilder":
        """
        Construye un subgrupo de recursos `null_resource` con un prefijo de nombre.

        Cada miembro del grupo sera una copia del prototipo base con un
        nombre renombrado a "{name}_{i}". El grupo se aniade al modulo compuesto
        como un submodulo bajo la clave `module` para que la exportacion quede anidada
        como: { "module": { name: [ { "resource": [...] } ] } }.
        """
        ...
        for i in range(size):
            def mutator(d: Dict[str, Any], idx=i) -> None:
                # Navega hasta el bloque null_resource y renombra la clave
                res_block = d["resource"][0]["null_resource"][0]
                original_name = next(iter(res_block.keys()))
                new_name = f"{name}_{idx}"
                res_block[new_name] = res_block.pop(original_name)

            clone_dict = proto.clone(mutator).data
            group.add(clone_dict)

        # Aniade el grupo exportado como un submodulo con el nombre dado
