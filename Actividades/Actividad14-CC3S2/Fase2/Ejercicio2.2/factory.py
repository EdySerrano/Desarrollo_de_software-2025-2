...
class TimestampedNullResourceFactory(NullResourceFactory):
    """
    Variacion de la fabrica que inserta un timestamp formateado en los triggers.
    Uso:
        TimestampedNullResourceFactory.create('name', '%Y%m%d')
    """

    @staticmethod
    def create(name: str, fmt: str) -> Dict[str, Any]:
        """Crea el mismo recurso que la fabrica base pero con `timestamp` dado por fmt."""
        ts = datetime.utcnow().strftime(fmt)
        # Forzamos el valor de 'timestamp' en los triggers; la fabrica base añadira
        # los demas triggers por defecto (factory_uuid) si no estan presentes.
        triggers = {"timestamp": ts}
        return NullResourceFactory.create(name, triggers=triggers)
