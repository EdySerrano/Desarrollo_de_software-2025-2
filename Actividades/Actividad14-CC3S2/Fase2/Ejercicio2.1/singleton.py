...
class ConfigSingleton(metaclass=SingletonMeta):
    ...
    def reset(self) -> None:
        """
        Limpia el diccionario de configuracion `settings` pero mantiene
        los metadatos de la instancia por ejemplo, `created_at` y `env_name`.

        Esto permite resetear la configuracion en tiempo de ejecucion sin
        recrear la instancia Singleton.
        """
        # Vacia el diccionario de settings en sitio para conservar referencias y evitar reemplazar el objeto si otras partes del codigo lo mantienen.
        self.settings.clear()
